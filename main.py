...
class Bot:
    ...
    async def button_handler(self, update: Update, context: CallbackContext):
        ...
        if callback == 'orders':
            await self.views['order_view'].show(update=update, context=context, user_id=user_id)
        ...
        if 'add_to_cart' in callback:
            ...
            await self.controllers['cart_controller'].add_product(product_id=product_id, user_id=user_id)
            await self.views['menu_view'].show(update, context)
    ...
    def main(self):
        ...
        # controllers
        admin_controller = AdminController(bot, self.ADMIN_CHAT_ID)
        conversation_controller = ConversationController(db, order_model, admin_controller)
