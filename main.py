...
class Bot:
    ...
    async def route_handler(self, update: Update, context: CallbackContext):
        ...
        if 'view' in route:
            ...
            if len(location_path := location.split('_')) > 1:
                location, obj_id = location_path
                context.user_data[f"{location}_id"] = obj_id
    ...
    def main(self):
        ...
        category_view = CategoryView(category_controller, navigation_controller, cart_controller, product_controller)
        self.views = {
            'start': start_view,
            'help': help_view,
            'menu': menu_view,
